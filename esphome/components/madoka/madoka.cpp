  ...
  switch (function_id) {
    ...
    case CMD_GET_SETPOINT:
      this->for_each_argument_(msg, [this](const FrameArgument &arg) {
        switch (arg.id) {
          case 0x20:
            // Setpoints are 16-bit big-endian, in 1/128 degree.
            if (arg.len >= 2) {
              this->cooling_setpoint_ = (float) arg.u16() / 128;
            }
            break;
          case 0x21:
            if (arg.len >= 2) {
              this->heating_setpoint_ = (float) arg.u16() / 128;
            }
            break;
          case 0x32:
            ...
            if (arg.len >= 1 && arg.value[0] != this->min_differential_) {
              this->min_differential_ = arg.value[0];
    ...
    case CMD_GET_FAN_SPEED: {
      ...
      this->for_each_argument_(msg, [this, &fan_mode](const FrameArgument &arg) {
        if (this->cur_status_.mode == 1) {
        } else if ((arg.id == 0x21 && arg.len == 1 && this->cur_status_.mode == 4) ||
                   (arg.id == 0x20 && arg.len == 1 && this->cur_status_.mode != 4)) {
          fan_mode = arg.value[0];
        }
      });
      switch (fan_mode) {
        case 0:
    ...
    case CMD_GET_SENSOR_INFORMATION:
      this->for_each_argument_(msg, [this](const FrameArgument &arg) {
        ...
        } else if (arg.id == 0x41 && this->outdoor_temperature_sensor_ != nullptr && arg.len >= 1) {
          // Sign-magnitude, 0xFF = no probe: see decode_outdoor_temperature.
          int value = 0;
          if (decode_outdoor_temperature(arg.value[0], value)) {
